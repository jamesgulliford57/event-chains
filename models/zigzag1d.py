import numpy as np
from models.pdmp import PDMP
import math 

class ZigZag1d(PDMP):
    def __init__(self, num_samples, final_time, event_time_func, x0=0.0, v0=1, dim=1, poisson_thinned=False, 
                 event_rate=None, event_rate_bound=None, **pi_params):
        super().__init__(num_samples=num_samples, final_time=final_time, event_time_func=event_time_func, 
...
class GaussZigZag1d(ZigZag1d):
    ...
    def __init__(self, num_samples, final_time, x0=0.0, v0=1, dim=1, poisson_thinned=False, 
                 event_rate=None, event_rate_bound=None, mu_pi=0, sigma_pi=1):
        super().__init__(num_samples=num_samples, final_time=final_time, event_time_func=self.event_time_func, 
    ...
    def event_time_func(self):
        ...
        component_to_flip = 0
        if a >= 0:
            return -a + math.sqrt(a**2 - 2 * math.log(1 - np.random.rand())), component_to_flip
        else:
            return -a + math.sqrt(-2 * math.log(1 - np.random.rand())), component_to_flip
